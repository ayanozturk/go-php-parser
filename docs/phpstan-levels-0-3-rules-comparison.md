...
6. Complete level-1 variable-flow precision for dynamically typed method receivers, full built-in output metadata, dynamic-variable effects, and non-literal `break`/`continue` levels. Resolved `$this`, `self`/`parent`, explicit static, known `new` receiver, constructor by-reference calls, and positive-integer multi-level loop/switch transfers are covered.
