...
1. Complete PHPStan 2.2.x level-0 rule parity across all registered rule classes, especially remaining modifier legality, constructor consistency, broader class constant legality, remaining enum edge cases (for example `Serializable`, non-literal case values), and PHPStan API restriction rules.
