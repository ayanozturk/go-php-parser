...
| 5 | Function, method, and constructor argument types | Partial, differential-gated | `A.ARG.TYPE` | Sixteen fixtures covering mismatches plus scalar, subtype, `self`, generic Collection, unbound template-return, `@phpstan-type` list-alias, InputBag query `get`, and Closure-filter controls. Failing cases are silent at level 4. |
...
| `A.ARG.TYPE` | Checks resolved named-function, method, and constructor argument value types against declared parameter types. | Partial PHPStan level 5 coverage, gated by sixteen fixtures. |
