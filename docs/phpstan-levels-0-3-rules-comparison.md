...
| 1 | Undefined and possibly undefined variables | Partial | `PHPStan.Level1.Variables` | Exact-span immutable facts distinguish undefined, possibly defined, and definitely defined reads. Joined analysis covers conditionals, short-circuit expressions, ternaries, bounded loop convergence, `switch`, `try`/`catch`/`finally`, globals/statics, destructuring, explicit by-value/by-reference closure captures, direct reference assignments, resolved by-reference function outputs, resolved `$this`, static, known-receiver, and constructor outputs, selected built-in output versus input/output parameters, numeric multi-level `break`/`continue`, known-string dynamic reads, conservative dynamic writes, constant-array and unknown-array `extract()` effects, `compact`, and `isset`/`empty` suppression. Twenty-two PHPStan 2.2.5 differential fixtures gate positive and clean controls. Dynamic calls, dynamic transfer levels, complex dynamic-name expressions, and complete built-in signatures remain incomplete. |
...
| `PHPStan.Level1.Variables` | Joined undefined-variable diagnostics for both always-undefined and possibly-defined reads. | Partial PHPStan level 1 coverage. |
...
6. Complete level-1 variable-flow precision for dynamically typed method receivers, full built-in output metadata, complex dynamic-name and `extract()` flag/prefix behavior, and non-literal `break`/`continue` levels. Resolved `$this`, `self`/`parent`, explicit static, known `new` receiver, constructor by-reference calls, positive-integer multi-level loop/switch transfers, known-string dynamic reads, and constant/unknown-array `extract()` effects are covered.
