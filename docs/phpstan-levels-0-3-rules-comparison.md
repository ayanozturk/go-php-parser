...
Executable parity evidence is tracked separately in [the analyser capability matrix](analyser-capability-matrix.md). Current differential packs gate 94 / 24 / 101 / 40 cases at levels 0–3, 46 cases at level 5, 24 at level 6, 8 at level 7, and 35 at level 8. The latest level-6 additions match PHPStan 2.1.44; the preceding complete packs were verified against PHPStan 2.2.5. Descriptive rows in this document are not parity claims unless backed by a checked-in differential fixture there.
...
| 3 | Return types, types assigned to properties | Partial, with thirty-five differential cases |
| 6 | Missing generic type arguments, iterable value types, and missing declaration types | Partial, exact differential-gated slice with twenty-two cases; broader missing-type parity remains open |
...
| --- | --- | --- | --- | --- |
| 0 | Basic semantic checks | Partial | `Level0.Language`, `Level0.ClassModel`, parser/command parse-error reporting | Covers selected language legality checks: duplicate literal array keys, undefined `goto` labels, literal include/require file existence, invalid `unset`/`void` casts, invalid increment/decrement targets, regex pattern validation, printf/sprintf placeholder count checks, and resolved non-throwable `throw` expressions. Full PHPStan basic-rule parity is not complete. |
| 0 | Unknown classes | Partial | `Level0.Symbols`, `Level0.ClassModel` | Covers unknown classes in `new`, `extends`, `implements`, interface `extends`, trait use, static calls, class constants/static properties, imports, type hints, catch types, and top-level attributes. Class constant checks now resolve inherited constants and report private/protected constant access and final constant overrides. File-level reflection guards now also suppress selected unknown class/function/const import and type-reference diagnostics after `class_exists`, `interface_exists`, `trait_exists`, `enum_exists`, `function_exists`, and `defined`. Still missing scope-sensitive guards and some parser/AST surfaces. |
| 0 | Unknown functions | Partial | `Level0.Symbols` | Covers ordinary function calls and `use function`, backed by project and curated built-in function indexes. Built-in coverage is intentionally partial. |
| 0 | Unknown methods called on `$this` | Partial | `Level0.Symbols` | Covers direct `$this->method()` calls against the current class/project symbol index, with visibility checks for private/protected methods using declaring classes. Also checks method calls on known receiver expressions (for example `new Foo()` and `Foo::class`). Dynamic methods and PHPStan's full magic-method behavior are not covered. |
| 0 | Wrong number of arguments passed to methods and functions | Partial | `Level0.Invocation`; legacy `A.ARG.COUNT` outside explicit level mode | In `analysis_level: 0`, checks ordinary functions, constructors (including inherited), static calls, `$this` and known-receiver method calls, named arguments, duplicate named arguments, positional-after-named, and unpack ordering for known signatures. Also reports private/protected constructor and method access using declaring classes and subclass checks, static call to instance methods, and instance call to static methods when the receiver class is known. Does not yet match PHPStan's full signature database or all dynamic/constant-array unpack cases. |
| 0 | Class/model legality | Partial | `Level0.ClassModel` | Covers duplicate class declarations, instantiating interface/trait/enum/abstract class, extending final/non-class/unknown classes, implementing non-interface/unknown interfaces, interface extends checks, trait-use validity, static call to instance method, selected property existence/staticness checks, final+abstract classes, abstract methods in non-abstract classes, invalid private/final abstract methods, overriding final parent methods and constants, constructor return types, non-public interface methods and constants, private final constants, `@phpstan-consistent-constructor` private-constructor and child-constructor compatibility checks, missing required methods from implemented interfaces or abstract parents, basic required-method signature compatibility for parameter counts/names and return type equality, readonly/non-readonly class inheritance legality, readonly class property legality (including promoted constructor params), readonly property override legality, enum sanity (backing type, case values, duplicate backed values, constructor/destructor, disallowed magic methods, native method redeclaration, and disallowed `Serializable` implementation), and invalid throw expressions for resolved non-throwable classes. Missing full variance/signature compatibility and additional modifier edge cases. |
| 0 | Type/reference legality | Partial | `Level0.Symbols`, `Level0.ClassModel` | Covers class-like type references in params, returns, properties, constants, interface methods, catches, imports, and top-level attributes. Does not yet cover every modern syntax location or PHPDoc references. |
| 0 | Native callable property types | Differential-gated | `Level0.PropertyCallableType` | Six fixtures cover direct, nullable, union, and promoted callable property failures plus PHPDoc callable and native `Closure` clean controls. |
...
| 3 | Return types | Partial, differential-gated | `A.RETURN.TYPE`, `A.RETURN.VOID`, `A.RETURN.NEVER` | Level-3 fixtures cover function/method mismatches, missing returns, `void` and `never` contracts, inferred arithmetic/comparison/logical/unary/spaceship results, clean scalar/nullable controls, method-level callable `@template T` binding, and `@extends` generic-parent method return inference. Coverage remains narrower than PHPStan because inference and symbol knowledge are limited. |
...
| 5 | Function, method, and constructor argument types | Partial, differential-gated | `A.ARG.TYPE` | Forty-six fixtures covering mismatches plus scalar, subtype, `self`, generic Collection, direct and aliased PHPDoc lists, unbound template-return, InputBag query `get`, Closure-filter, assignment `@var`, conditional `createForm` return, interface method-template, and flow-narrowing controls. Failing cases are silent at level 4. |
| 6 | Missing generic, iterable, and declaration types | Partial, differential-gated | `Level6.MissingGenericType`, `Level6.MissingIterableValueType`, `Level6.MissingParameterType`, `Level6.MissingReturnType`, `Level6.MissingPropertyType` | Twenty-four fixtures cover missing generic arguments and iterable value types, missing named-function and method parameter/return types, a missing property type, and clean generic, typed-array, array-shape, named and anonymous inherited contracts, enum-method, inline promoted-parameter, explicit `mixed`/`void`, PHPDoc declaration, and typed-constructor controls. The ten failing cases are silent at level 5; aliases, conditional/dynamic types, extension-provided signatures, and other declaration surfaces remain outside this exact gate. |

## Currently Implemented Analysis Rules

| Rule code | Description | Closest PHPStan level mapping |
| --- | --- | --- |
| `Level0.Symbols` | Groups PHPStan level-0 compatibility checks for unknown symbols, class-like references, imports, static calls, `$this` calls, constants, attributes, and selected property existence checks. | Partial PHPStan level 0 coverage. Enabled only when the selected analysis level includes 0. |
| `Level0.ClassModel` | Internal diagnostic code emitted by the level-0 rule group for class hierarchy/model legality. | Partial PHPStan level 0 coverage. |
| `Level0.Invocation` | Internal diagnostic code emitted by the level-0 rule group for argument-count and named-argument validity. | Partial PHPStan level 0 coverage. |
| `Level1.Variables` | Joined undefined-variable diagnostics for both always-undefined and possibly-defined reads. | Partial PHPStan level 1 coverage. |
| `Level0.Language` | Internal diagnostic code emitted by the level-0 rule group for selected language legality checks. | Partial PHPStan level 0 coverage. |
...
| `A.ARG.TYPE` | Checks resolved named-function, method, and constructor argument value types against declared parameter types. | Partial PHPStan level 5 coverage, gated by forty-six fixtures. |
...
1. Complete PHPStan 2.2.x level-0 rule parity across all registered rule classes, especially remaining modifier legality, broader class constant legality, deeper constructor-signature variance, remaining enum edge cases (for example non-literal case values), and PHPStan API restriction rules.
...
6. Complete level-1 variable-flow precision for dynamically typed method receivers, extension-dependent built-in output metadata, complex dynamic-name and `extract()` flag/prefix behavior, and non-literal `break`/`continue` levels. Resolved `$this`, `self`/`parent`, explicit static, known `new` receiver, constructor by-reference calls, core/standard built-in output and input/output references, positive-integer multi-level loop/switch transfers, known-string dynamic reads, and constant/unknown-array `extract()` effects are covered.
7. PHPStan level 1 magic method/property diagnostics.
