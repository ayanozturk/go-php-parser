...
| 5 | Function, method, and constructor argument types | Partial, differential-gated | `A.ARG.TYPE` | Eighteen fixtures covering mismatches plus scalar, subtype, `self`, generic Collection, unbound template-return, `@phpstan-type` list-alias, InputBag query `get`, Closure-filter, and assignment `@var` controls. Failing cases are silent at level 4. |
...
| `A.ARG.TYPE` | Checks resolved named-function, method, and constructor argument value types against declared parameter types. | Partial PHPStan level 5 coverage, gated by eighteen fixtures. |
