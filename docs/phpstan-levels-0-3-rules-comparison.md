...
6. Complete level-1 variable-flow precision for dynamically typed method receivers, full built-in output metadata, dynamic-variable effects, and cross-scope multi-level loop transfers. Resolved `$this`, `self`/`parent`, explicit static, known `new` receiver, and constructor by-reference calls are covered.
