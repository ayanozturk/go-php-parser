...
| 1 | Undefined and possibly undefined variables | Partial | `PHPStan.Level1.Variables` | Exact-span immutable facts distinguish undefined, possibly defined, and definitely defined reads. Joined analysis covers conditionals, short-circuit expressions, ternaries, bounded loop convergence, `switch`, `try`/`catch`/`finally`, globals/statics, destructuring, closures, `compact`, and `isset`/`empty` suppression. PHPStan 2.2.5 differential fixtures gate always-undefined, branch-possible, loop-possible, exhaustive-branch clean, and do/while clean cases. By-reference writes, precise closure `use` capture, and cross-scope multi-level `break`/`continue` remain incomplete. |
...
6. Complete level-1 variable-flow precision for by-reference writes, explicit closure `use` capture, dynamic-variable effects, and cross-scope multi-level loop transfers.
