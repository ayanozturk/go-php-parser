...
Progress: the production `analyze` command now parses each selected file once, constructs one immutable `SemanticSnapshot`, runs the same registered analysis engine used by PHP Strom, emits deterministic diagnostics, accounts for parser/read failures, and returns stable clean/finding/infrastructure exit codes. PHP Strom pins engine commit `6df9189`, constructs dependency-revisioned document snapshots over its workspace project index, consumes their shared facts, flow graph, and variable-flow reader, replaces changed file contributions through the parser's immutable incremental index API, gates its synthetic editor path with bounded latency/accounting traces, maps structured analysis spans to half-open UTF-16 LSP ranges, and surfaces DNF, nullable, callable-return, declared-callable, nested/list array-shape, clone, coalesce, match, nullsafe, template class-string, and non-object receiver method diagnostics through its existing undefined-symbol and type-error configuration. Its pinned and sibling-development validation paths both pass. Literal end-to-end pipeline equivalence remains incomplete because the extension still owns workspace overlays, stubs, PHP-version/configuration mapping, scheduling, mixed style-rule coordinates, unstructured parser errors, and bounded conservative dependency matching.
...
29. **Done — infer dynamic array-shape and list indexes at level 2.** Assigned constant string/int keys, concatenated string literals, `self`/`static`/same-class constants, ternary unions of those keys, unknown `string`/`mixed` indexes (union of all callable fields), and unknown `int`/`float` indexes (numeric keys only) now produce the same `method.notFound` receivers as pinned PHPStan. Missing literal keys, `bool` indexes, and int indexes into purely named shapes stay conservative. Eight fixtures bring the executable gates to 63/24/57/1 and match PHPStan `2.2.x-dev@e4ab62a`. Per-alternative DNF availability remains a separate higher reference level.
