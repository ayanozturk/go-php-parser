...
1. **Maintenance:** rewrite `vscode-php-strom/FEATURES.md` for the Go language server and `go-php-parser`; decide whether to delete `src/server`. Then structured parser errors and style-rule range migration. Remaining silent or extra-identifier level-0 leftovers (inherited parameter names, native enum method redeclaration) stay out of the level-0 pack.
2. **PHPStan identifier reclassification:** unknown methods on nullable classes stay `method.notFound` here; PHPStan emits `method.nonObject` at level 8. Do not fold that into the known-method nullable pack.
...
Completed deliveries 1–34 are archived below and are not the current queue.
