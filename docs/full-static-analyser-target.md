...
Progress: the production `analyze` command now parses each selected file once, constructs one immutable `SemanticSnapshot`, runs the same registered analysis engine used by PHP Strom, emits deterministic diagnostics, accounts for parser/read failures, and returns stable clean/finding/infrastructure exit codes. PHP Strom pins engine commit `ad09a14`, constructs dependency-revisioned document snapshots over its workspace project index, consumes their shared facts, flow graph, and variable-flow reader, replaces changed file contributions through the parser's immutable incremental index API, gates its synthetic editor path with bounded latency/accounting traces, maps structured analysis spans to half-open UTF-16 LSP ranges, and surfaces the DNF-preserving nullable receiver method rule through its existing undefined-symbol configuration. Its pinned and sibling-development validation paths both pass. Literal end-to-end pipeline equivalence remains incomplete because the extension still owns workspace overlays, stubs, PHP-version/configuration mapping, scheduling, mixed style-rule coordinates, unstructured parser errors, and bounded conservative dependency matching.
