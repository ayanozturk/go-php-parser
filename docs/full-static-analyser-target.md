...
- Current production pin: parser `a92ff39` (`v0.0.0-20260831160450-a92ff395870a`), extension `960f927`, package `0.1.33`
...
34. **Done — detect known methods on nullable object receivers at PHPStan level 8.** `PHPStan.Level8.MethodNonObject` reports `?Service->execute()`, class-or-null ternaries, nullable intersections, and `?object` while leaving nullsafe `?->` clean. Scalar `int|null` and unknown nullable methods stay on the level-2 rules. Five fixtures bring the executable gates to 88/24/65/1/5/5 and match PHPStan `2.2.x-dev@e4ab62a`. `MethodCallNode` now records nullsafe operators. The editor maps the new code through the type-error setting.
