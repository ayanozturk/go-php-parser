...
Progress: the production `analyze` command now parses each selected file once, constructs one immutable `SemanticSnapshot`, runs the same registered analysis engine used by PHP Strom, emits deterministic diagnostics, accounts for parser/read failures, and returns stable clean/finding/infrastructure exit codes. PHP Strom now pins engine commit `5c7bfc5` and its pinned and sibling-development validation paths both pass. Literal end-to-end pipeline equivalence remains an extension-side M1 integration because the extension still supplies its own workspace overlay, stubs, PHP version, disabled-code context, and resolver without consuming the snapshot's shared facts and flow graph.
...
11. **Done — advance and validate the PHP Strom production pin.** Extension commit `56e2da2` pins engine commit `5c7bfc5`, updates undefined-variable suppression for the level-1 diagnostic code while retaining level-0 compatibility, and adds the matching contract coverage. Pinned and sibling-development Go tests, vet, and race suites pass; TypeScript lint/compile/package, the VS Code extension-host suite, all six server target builds, and the npm vulnerability audit pass. This delivers the current engine revision but does not yet make PHP Strom consume the engine's shared semantic snapshot or incremental CLI cache.
