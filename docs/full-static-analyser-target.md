...
5. **In progress — design the immutable semantic snapshot and shared fact-store interfaces.** The first compatibility layer is now implemented in `analyse/semantic_snapshot.go`: `SemanticSnapshot` owns the mutable `ProjectIndex` behind a read-only `SymbolResolver`, returns defensive copies of slice-backed symbol metadata, assigns stable case-insensitive semantic IDs to resolved classes/methods/properties/functions, and exposes a deterministic byte-span-keyed `SemanticFactReader`. Fact construction rejects invalid spans and duplicate keys rather than allowing order-dependent overwrites. Adversarial tests cover input/result mutation isolation, stable ordering and IDs, and concurrent reads. Existing `ProjectIndex` callers remain source-compatible so rule migration can proceed incrementally. Remaining work: attach declaration spans and IDs during indexing, migrate analysis caches/rules onto shared facts, and replace direct mutable-map access inside the `analyse` package.
