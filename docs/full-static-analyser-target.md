...
28. **Done — detect method calls on non-object receivers at level 2.** A separate `PHPStan.Level2.MethodNonObject` rule reports `int`/`array`/`callable`/`iterable` and class-or-scalar unions when the class alternative lacks the method, matching PHPStan `method.nonObject`. `object` receivers, `mixed`, and known methods on class-or-scalar unions stay clean. Eight fixtures bring the executable gates to 63/24/49/1 and match PHPStan `2.2.x-dev@e4ab62a`. Dynamic indexes and per-alternative DNF stay outside this gate.
