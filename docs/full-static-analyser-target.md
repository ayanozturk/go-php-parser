...
Progress: the production `analyze` command now parses each selected file once, constructs one immutable `SemanticSnapshot`, runs the same registered analysis engine used by PHP Strom, emits deterministic diagnostics, accounts for parser/read failures, and returns stable clean/finding/infrastructure exit codes. PHP Strom pins engine commit `5c7bfc5`, constructs revision-aware document snapshots over its workspace project index, and consumes their shared facts, flow graph, and variable-flow reader in diagnostics and semantic providers. Its pinned and sibling-development validation paths both pass. Literal end-to-end pipeline equivalence remains incomplete because the extension still owns workspace overlays, stubs, PHP-version/configuration mapping, scheduling, and a whole-project rebuild path that differs from the standalone CLI's incremental cache.
...
12. **Done — consume shared semantic snapshots in PHP Strom.** Extension commit `15abf7a` introduces an explicit workspace project revision and caches parser-native per-document snapshots by exact text plus that revision. Diagnostics, hover, definition, and declaration now share the snapshot's facts, control-flow graph, and variable-flow reader while preserving the workspace symbol fallback. Document edits and cross-file project rebuilds invalidate cached semantic state; closed/background scans use transient snapshots and `didClose` releases interactive state, avoiding unbounded workspace retention discovered during race validation. Full pinned and sibling tests/vet/race, all six server builds, TypeScript lint/compile/package, extension-host tests, and the npm audit pass. The next integration batch is true incremental project-index/dependant invalidation plus trace-based editor latency evidence; the standalone CLI cache is not directly reused by the LSP.
