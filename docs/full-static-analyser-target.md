...
The next M1 slice added immutable exact-span variable-read facts with three joined states (undefined, possibly defined, and definitely defined), branch joins, bounded loop convergence, and joined handling for `switch`, `try`/`catch`/`finally`, short-circuit expressions, ternaries, globals/statics, destructuring, closures, `compact`, and `isset`/`empty`. The level-aware rule now reports both undefined and possibly-defined reads as `PHPStan.Level1.Variables`. A live PHPStan 2.2.5 differential exposed and corrected the previous roadmap assumption that always-undefined variables belonged to level 0: PHPStan 2.2.5 emits neither case at level 0 and emits `variable.undefined` for both at level 1. The checked-in gates now contain four level-0 cases and five level-1 cases, including clean exhaustive-branch and `do`/`while` controls.

The first implementation eagerly cloned whole variable maps and allocated 5.27GB over a three-iteration WordPress profile. Copy-on-write state, implicit predefined-variable facts, compact internal read storage, node-keyed fact deduplication, and allocation-free internal fact iteration reduced that profile to 1.88GB (approximately 64% below the first implementation), but the correctly rebuilt previous engine allocated 1.25GB over the same three iterations. Exact read-fact construction remained the candidate's largest allocator, followed by copy-on-write detachment and joins.

A final ten-round interleaved process-cold indicator accounted for 5,362/5,362 files, 1,451,710 LOC, 47,359,328 bytes, and zero failures on every run. Candidate cold mean/median were 726.3/723ms versus 645/632ms for the previous engine; candidate CV passed at 4.22%, but baseline CV failed at 8.67%, so the comparison is rejected. The validation runs observed approximately 823MB candidate peak RSS versus 643MB baseline, and candidate diagnostics were stable at 21,082 versus 25,889 for the previous engine. The 4,807-diagnostic reduction is an expected semantic change from replacing the old branch-union walk, not evidence of improved correctness beyond the five reviewed level-1 fixtures. This indicator therefore supports retaining copy-on-write joined state but changing the next design slice to avoid eagerly materialising every definitely-defined read and to share traversal/facts with the broader semantic pass before adding more flow domains. The generated benchmark JSON and heap profiles remain under `/tmp` and are not committed.
...
10. **M1 progress — joined variable-flow facts and PHPStan level 1 undefined variables.** `SemanticSnapshot` now exposes deterministic immutable variable-read facts through `AnalysisContext`; the registered rule consumes the same facts without defensive-copy allocation internally. Copy-on-write joined states cover conditionals, loops, `switch`, `try`/`catch`/`finally`, short-circuit expressions, ternaries, globals/statics, destructuring, closures, `compact`, and suppression contexts. Differential evidence against PHPStan 2.2.5 corrected always-undefined variables from level 0 to level 1. Remaining precision work includes by-reference writes, explicit closure `use` capture, dynamic-variable effects, and cross-scope multi-level `break`/`continue`. The early WordPress indicator also makes eager storage of every definitely-defined read the next allocation target before another flow domain is added.
