...
- Executable PHPStan gates: 63 / 24 / 65 / 1 (levels 0–3) vs `2.2.x-dev@e4ab62a`
...
Progress: spans, immutable snapshots, allocation-light resolver views, copy-on-write function-scope maps, CFG slices, incremental indexing, and PHPStan-gated packs through levels 0–3 are in place. Level 0 is **partial** (63 reviewed fixtures, not corpus parity). Levels 1–3 are gated but thin (24 / 65 / 1). Same-host WordPress cold runs finish inside the 60s / 2 GB envelope with full file accounting, but CV has not met the 5% contract, so the timing gate is not claimed. Remaining M1 work is correctness coverage first, then an isolated-host measurement.
...
1. **Ungated level-0 surfaces that already have unit tests.** Expand the differential pack across remaining class-model, import, invocation, and language rows in `docs/analyser-capability-matrix.md`.
...
Completed deliveries 1–30 are archived below and are not the current queue.
...
30. **Done — infer remaining level-2 expression-form receivers.** Parser commit pending pin resolves file-level constants, other same-file class constants, `match` indexes, property `@var` shapes (including native `array` hints), method `@return` shapes, static properties, and `list{Class}` object indexes to the same `method.notFound` receivers as pinned PHPStan. Eight fixtures bring the executable gates to 63/24/65/1 and match PHPStan `2.2.x-dev@e4ab62a`. Per-alternative DNF availability remains a separate higher reference level.
