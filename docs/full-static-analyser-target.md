...
- Current production pin: parser `e629d42` (`v0.0.0-20260831163118-e629d4267f4c`), extension `8348748`, package `0.1.34`
...
### Persistent function-scope layers, 2026-08-31

Variable and property flow types now use bounded persistent layers instead of eager whole-map copy-on-write detachment. Branch clones share immutable parents, use inline one-entry deltas, allocate a local map only for additional distinct writes, and compact at a fixed depth. Missing callable and array-shape deletion avoids unnecessary detachment. Isolation and boundedness tests cover original, child, sibling, chained, cached-class-property, large-base, and sequential-write cases.
...
### Duplicate method-parameter copy removal, 2026-09-01
...
### Allocation-light method resolver views, 2026-09-01

Internal analysis now borrows immutable method metadata through a private resolver view for existence, visibility, argument-count, and symbol-ID queries. The view walks inheritance with a stack-allocated seen set, lowercases the method name once, and reuses index-owned parameter storage. Type-sensitive inference still uses `ResolveMethod` / `ResolveMethodWithGenerics` so inherited generic bindings keep their defensive copies. Against exact baseline `9a4f4a4`, three-pass WordPress allocated space fell from 4.77GB to 4.50GB, a 5.7% reduction, and the previous approximately 0.79GB public `ResolveMethod` path shrank to 0.23GB. The twenty-round interleaved cold comparison is rejected because candidate CV was 26.9% and baseline CV was 9.0%. Durable details are in `docs/benchmarks/2026-09-01-wordpress-method-resolver-view.md`; generated evidence remains local.
...
Progress: spans, immutable snapshots, allocation-light resolver views, copy-on-write function-scope maps, CFG slices, incremental indexing, and PHPStan-gated packs through levels 0–3, 7, and 8 are in place. Level 0 is **partial** (88 reviewed fixtures, not corpus parity). Levels 1–3, 7, and 8 are gated but thin (24 / 65 / 1 / 5 / 5). Same-host WordPress cold runs finish inside the 60s / 2 GB envelope with full file accounting, but CV has not met the 5% contract, so the timing gate is not claimed. Remaining M1 work is Mago-class performance on the snapshot-backed pipeline.
...
1. **Optimize from profiles, not from diagnostic cuts.** Persistent scope layers removed `copyTypeMap`; private function views removed public function-result copying; the snapshot facade no longer duplicates method parameter copies; remaining scope metadata is copy-on-write; internal method lookups reuse index-owned parameter storage; mixed-case ASCII identifier folds are interned; narrowing facts insert in place; and control-flow graphs keep common successor edges inline. Next: reduce semantic-fact insertion, remaining per-clone scope object cost, and leftover control-flow reachability maps. Do not disable rules or skip `vendor` to win the comparison.
...
33. **Done — detect unknown methods on partial unions at level 7.** Parser commit `a92ff39` adds `Level7.MethodUnion` for receivers where some but not all DNF alternatives provide the method, matching PHPStan `method.notFound` without duplicating all-missing level-2 diagnostics. Five fixtures bring the executable gates to 88/24/65/1/5 and match PHPStan `2.2.x-dev@e4ab62a`. Extension commit `960f927` pins pseudo-version `v0.0.0-20260831160450-a92ff395870a` and gates editor behavior through the undefined-symbol setting. Nullable `method.nonObject` stays at level 8.
34. **Done — detect known methods on nullable object receivers at level 8.** Parser commit `e629d42` adds `Level8.MethodNonObject` for `?Service->execute()`, class-or-null ternaries, nullable intersections, and `?object`, while leaving nullsafe `?->` clean. Scalar `int|null` and unknown nullable methods stay on the level-2 rules. Five fixtures bring the executable gates to 88/24/65/1/5/5 and match PHPStan `2.2.x-dev@e4ab62a`. `MethodCallNode` now records nullsafe operators. Extension commit `8348748` pins pseudo-version `v0.0.0-20260831163118-e629d4267f4c` and gates editor behavior through the type-error setting.
35. **Done — measure Mago-class performance on the production snapshot path.** Parser commit `3155c07`. `cmd/benchmark` full analysis now builds one `SemanticSnapshot` per run and analyses through `snapshot.NewAnalysisContext()`, matching `analyze` and PHP Strom. The CLI cold path reuses the already-built project index instead of constructing a second one inside `NewSemanticSnapshotScoped`. A WordPress profile of that path (5,357/5,357 files, 26,321 diagnostics) was GC-dominated. Follow-ups keep that diagnostic count while combining the level-2/7/8 method-receiver walks, caching parsed source for empty-statement analysis, reusing file/namespace type context across `walkAll`, folding ASCII identifier keys without `unicode.ToLower`, scanning empty-statement source as a view over file bytes instead of `strings.Split`, sharing the argument type/count walk (`4c168ae`), visiting level-0 type/symbol/language checks in one walk (`abb6633`), visiting method-visibility and throw-type checks in one walk (`1c5998b`), and collecting deprecated-call issues during the shared argument walk. Diagnostic packs stay frozen; further speed work comes from profiles of this pipeline, not from cutting rules.
36. **Done — borrow immutable method metadata for internal lookups.** Private `resolveMethodView` / `resolveOwnMethodView` queries reuse index-owned parameter storage, walk inheritance without a heap `seen` map, and lowercase the method name once. Public `ResolveMethod` still clones parameters and applies generic bindings. Existence, visibility, argument-count, and snapshot symbol-ID paths use the view; type inference keeps the copying resolver. A three-iteration WordPress profile against exact `9a4f4a4` retained 5,357/5,357 files and 22,387 diagnostics while allocated space fell from 4.77GB to 4.50GB. The interleaved cold comparison is rejected on CV. See `docs/benchmarks/2026-09-01-wordpress-method-resolver-view.md`.
