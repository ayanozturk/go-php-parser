...
10. **M1 progress — joined variable-flow facts and PHPStan level 1 undefined variables.** `SemanticSnapshot` now exposes deterministic immutable variable-read facts through `AnalysisContext`; the registered rule consumes a compact diagnostic subset without defensive-copy allocation, while complete read facts are materialised lazily for public callers. Copy-on-write analyser-local variable slots cover conditionals, loops, `switch`, `try`/`catch`/`finally`, short-circuit expressions, ternaries, globals/statics, destructuring, explicit closure captures, reference assignment, resolved by-reference function, method, static, and constructor outputs, `compact`, and suppression contexts. Differential evidence against PHPStan 2.2.5 corrected always-undefined variables from level 0 to level 1 and now covers thirteen fixtures. An early method-resolution allocation regression was caught and reduced through a narrow immutable reference-parameter query; all interleaved timing results remain rejected on variance. Remaining precision work includes dynamically typed receivers, complete built-in output metadata, dynamic-variable effects, and cross-scope multi-level `break`/`continue`.
