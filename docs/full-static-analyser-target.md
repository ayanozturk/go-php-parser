...
- **M1 progress (first control-flow graph slice):** `SemanticSnapshot` now owns immutable statement-level control-flow graphs keyed by file and exact lexical-scope spans. `FlowGraphReader` exposes defensive graph values, statement reachability, and normal-fallthrough queries through `AnalysisContext`; registered-rule context cloning preserves the reader. `Generic.CodeAnalysis.UnreachableCode` consumes snapshot reachability while retaining its legacy fallback and exact diagnostics when no graph is available. The first slice deliberately models compound statements as atomic blocks with separate child-scope graphs, preserves conservative loop behavior, and treats duplicate or missing spans as unknown rather than aliasing facts. Adversarial coverage verifies exhaustive and partial branches, nested unreachable code, deterministic IDs, defensive copies, ambiguous spans, and concurrent reads. Full branch-edge expansion, `try`/`switch`/`do`, and loop-control edges remain before the graph can support joined variable-state analysis.
