...
Progress: the production `analyze` command now parses each selected file once, constructs one immutable `SemanticSnapshot`, runs the same registered analysis engine used by PHP Strom, emits deterministic diagnostics, accounts for parser/read failures, and returns stable clean/finding/infrastructure exit codes. PHP Strom pins engine commit `f1e06b9`, constructs revision-aware document snapshots over its workspace project index, consumes their shared facts, flow graph, and variable-flow reader, and replaces changed file contributions through the parser's immutable incremental index API. Its pinned and sibling-development validation paths both pass. Literal end-to-end pipeline equivalence remains incomplete because the extension still owns workspace overlays, stubs, PHP-version/configuration mapping, scheduling, and conservative global invalidation after exported changes.
