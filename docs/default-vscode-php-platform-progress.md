...
- Engine revision validated and consumed by PHP Strom: pushed commit `ad09a14`.
- Extension checkout: `/Users/ayan/Projects/vscode-php-strom`, `main` at pushed commit `5ea6c4a`; the current package version is `0.1.29`.
- Production extension builds pin `github.com/ayanozturk/go-php-parser` at pseudo-version `v0.0.0-20260831071512-ad09a14bf44f`. `make test-server-dev` validates the same engine through the generated, ignored sibling-workspace path.
...
1. **Correctness:** extend receiver facts to variable callable returns and dynamic `class-string<T>` construction, then cover other non-object branches and separately gate higher-level per-alternative DNF availability.
