...
- **Checked-in differential pack:** 28 cases in `testdata/diagnostic-differential-level8`.
...
The rule does not model every dynamic call, magic method, or unresolved union. The `known-method-negated-instanceof-or` clean control proves a known method on the right of `!$x instanceof T || …` after false-scope narrowing. Levels 0 through 7 are cumulative.
