...
// UseDeclImport describes one flattened use-import (class/function/const)
// from a KindUseDecl, matching lowerUseDecl / lowerUseClause path+type.
type UseDeclImport struct {
	Type string // "class", "function", or "const"
	Path string // trimmed, no leading/trailing backslash
}
...
func forEachUseClauseImport(file *File, green *GreenNode, offset int, prefix, useType string, fn func(UseDeclImport)) {
	...
	forEachChildDescGreen(green, offset, func(g *GreenNode, off int) bool {
		switch {
		case g.Kind() == KindToken && g.TokenType() == token.T_FUNCTION:
			itemType = "function"
		case g.Kind() == KindToken && g.TokenType() == token.T_CONST:
			itemType = "const"
		case isNameKind(g.Kind()):
			if nameGreen == nil {
				nameGreen, nameOff = g, off
			}
		case g.Kind() == KindUseGroup:
			groupGreen, groupOff = g, off
		}
		return true
	})
	if groupGreen != nil {
		base := strings.TrimPrefix(nameTextAt(file, nameGreen, nameOff), `\`)
		if prefix != "" {
		...
		forEachChildDescGreen(groupGreen, groupOff, func(g *GreenNode, off int) bool {
			...
			forEachUseClauseImport(file, g, off, base, itemType, fn)
			return true
		})
		return
	}
	path := strings.TrimPrefix(nameTextAt(file, nameGreen, nameOff), `\`)
	if prefix != "" {
